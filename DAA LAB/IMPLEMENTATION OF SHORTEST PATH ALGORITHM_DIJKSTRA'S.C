...
{ 
    ...
    /* Initialization: all distances to INF, all nodes marked unvisited */ 
    for (int i = 0; i < V; i++) 
    { 
        dist[i] = INF; 
        visited[i] = 0; 
    } 
    dist[0] = 0; // Starting from Source Vertex 0
    for (int count = 0; count < V; count++) 
    { 
        // Identify the vertex u with the minimum distance 
        int u = extractMin(dist, visited); 
        if (u == -1) 
            break; 
        visited[u] = 1; // Mark as processed
       /* Check for relaxation of edges connected to u */ 
        for (int j = 0; j < V; j++) 
        { 
